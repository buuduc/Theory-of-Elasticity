...
class GUI(tk.Frame):
    ...
    def importinput(self):
        ...
        self.E=tk.Entry(ipip,width=5)
        self.v1=tk.Label(ipip,text="nhập v: ")
        self.v=tk.Entry(ipip,width=5)
...
#        v.grid(column=1,row=2)
    ...
    def showinstruct(self):
        ...
        def WDUS(self):
            self.first.forget()
            self.output.forget()
            self.checkevent("us",False)
        def WDBD(self):
            self.first.forget()
            self.output.forget()
            self.checkevent("bd",False)
    ...
    def checkevent(self,H,A=False):
            ...
            if H=="us":
                ...
                self.ungsuatmain()
                if (self.check.get()==1):
                    print("giá trị test",float(self.v.get()))
                    
                    self.BD=biendang(self.US,float(self.E.get()),float(self.v.get()))
                    self.BD=self.BD.get_bd()
            elif H=="bd":
                if A:
                ...
                self.biendangmain()
                if (self.check.get()==1):
                    self.US=ungsuat(self.BD,float(self.E.get()),float(self.v.get()))
                    self.US=self.US.get_us()
