...
class GUI(tk.Frame):
    ...
    def choosen(self):
        ...
        a=tk.Checkbutton(khung,var=self.M,command= lambda: self.bind())
        a.pack(khung,side="left")
        def bind(self):
            if self.M.get()==1:
                tk.Label(khung,text="Nhập ma trận ứng suất")
            elif self.M.get()==0:
                tk.Label
...
#            self.test1.grid(row=1,column=1)
            if H=="us":
                ...
                self.test.grid(row=1,column=0)    
                if (self.check.get()==1):
                    c=biendang(self.US,float(self.E.get()),float(self.v.get()))
                    self.BD=c.get_bd()
                    self.biendangmain()
            elif H=="bd":
                ...
                self.test1.grid(row=1,column=1)
                if (self.check.get()==1):
                    c=ungsuat(self.BD,float(self.E.get()),float(self.v.get()))
                    self.US=c.get_us()
                    self.ungsuatmain()
